...
var businessTable={
    ...
    iconTag : function() {
        ...
        for (var i = 0; i < businessIconList.length; i++) {
            ...
            businessIconList[i].addEventListener ("click", function (e) {
    ...
    seachTag : function () {
         var businessSearchList = document.getElementsByTagName('business-search');
           for ( var i = 0; i < businessSearchList.length; i++ ) {
              ...
              var search = businessSearchList[i]; //businessSearchList[i].target || businessSearchList[i].srcElement || businessSearchList[i].originalTarget;             
             // var table = search.parentNode || search.srcElement|| search.originalTarget;
              //var businessTitleList = table.getElementsByTagName('business-title');  
             
              var searchHtml  = '<input type="search">'  //'<input type="button" value="Search">'          
    ...
    seachEvent : function(e){
                        ...
                        for( var k =0; k < businessBodyList.length; k++){
                            ...
                            if(searchbox.value ==='' || bodyFirstIndex  !=-1 || dataFirstIndex !=-1){
                                ...
                                // businessBodyList[k].style.display = ''; 
                                    businessRowList[k].style.display = ''; 
                            } else {
                                    //businessBodyList[k].style.display = 'none'; 
    ...
    filterTag : function () {
        var businessFilterList = document.getElementsByTagName('business-filter');
           for ( var i = 0; i < businessFilterList.length; i++ ) {
            var filter = businessFilterList[i]; //businessSearchList[i].target || businessSearchList[i].srcElement || businessSearchList[i].originalTarget;
                        
            var table = filter.parentNode || filter.srcElement|| filter.originalTarget;
            var businessTitleList = table.getElementsByTagName('business-title');  
            var filterHtml =  filter.innerHTML + '';//<label>Filter By:</label>';
            //  var filterHtml  = '<select>'   
                  for ( var j = 0 ; j < businessTitleList.length ; j++)
                  {
                      //filterHtml+= '<option value="'+ businessTitleList[j].innerText +'" businessFilterChecked>';
                           filterHtml+=  '<input type="checkbox" value="'+ j +'" checked>'+ businessTitleList[j].textContent;//innerText;
                      //filterHtml+= businessTitleList[j].innerText+ '</option>';
                  }                  
